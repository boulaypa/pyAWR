...
def main(args):
    ...
    sqlfile = os.environ.get('AWR_MINER_HOME') + "/sql/" + "awr_sql_id.sql"
    ...
    with open(sqlfile, 'r') as file:
            sql = file.read()
    sql2exec=sql.replace('__METRICS_NAME__', metrics).replace('__START_DATE__', start ).replace('__END_DATE__', end).replace('__SCHEMA__', schema).replace('__SQL_ID__', sql_id)
    ...
    pdf = df.pivot_table(index='SNAP_TIME', columns='SRC', values='METRICS', fill_value=0 )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sql_id", required=True )
