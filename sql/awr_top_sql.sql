...
sqlstat AS   (    SELECT stat.dbid,  
                         ...
                         ( stat.cpu_time_delta / 1000000 ) cpu_time_delta_sec ,  
                         ( stat.elapsed_time_delta / 1000000 ) elapsed_time_delta_sec,  
                         stat.fetches_delta,  
                         ...
                         CASE  
                           WHEN stat.executions_delta > 0 THEN  
                           ( stat.cpu_time_delta / 1000000 ) / stat.executions_delta  
                           ELSE 0  
                         END                       cpu_time_x_delta_sec,  
                         CASE  
                           WHEN stat.executions_delta > 0 THEN  
                           ( stat.elapsed_time_delta / 1000000 ) / stat.executions_delta  
                           ELSE 0  
                         END                       elapsed_time_x_delta_sec,  
                         CASE  
                         ...
                         CASE  
                           WHEN stat.executions_delta > 0 THEN  
                           stat.apwait_delta / stat.executions_delta  
                           ELSE 0  
                         END                       apwait_x_delta
                  ...
                  WHERE  stat.snap_id = snaps.snap_id  
                         ...
                         AND stat.parsing_user_id = u.user_id
                         -- AND stat.parsing_schema_name like 'ESBBWPX1%'
                         AND u.username != 'SYS'
                         ),  
 top  
...
plandate  
      AS (SELECT *  
          FROM   (SELECT DISTINCT sql_id, plan_hash_value, cost,  
                         MIN(TIMESTAMP) OVER(PARTITION BY sql_id, plan_hash_value, cost ) plan_date
...
snap_top_cp AS (  
 ...
 SELECT s.begin_interval_time time ,  
        ...
        TO_CHAR(rank, '09' ) ||'# '|| 
        TO_CHAR(top_pct, '09' )||'% '||
        s.sql_id ||' '||TO_CHAR(s.plan_hash_value, '999999999999999')||' '||
                        TO_CHAR(s.plan_date,'YYYYMONDD')||' '||
                        TO_CHAR(s.cost, '99999999')||' '||
                        username as src,  
        s.rank as rank,  
