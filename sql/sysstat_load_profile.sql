select snap_id, TO_CHAR(begin_interval_time,'YYYYMMDDHH24MI') AS snap_time, stat_name,  sum(delta_value) value from 
    ...
    where hss.snap_id=hs.snap_id 
        ...
        and hss.stat_name in ('db block changes','sorts (disk)','execute count','logons cumulative','user calls','parse count (total)','parse count (hard)','redo size','physical read total bytes','physical write total bytes','session logical reads','parse time cpu','CPU used by this session')) 
