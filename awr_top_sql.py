...
def main(args):
    ...
    sqlfile = os.environ.get('AWR_MINER_HOME') + "/sql/" + "awr_top_sql.sql"
    ...
    with open(sqlfile, 'r') as file:
            sql = file.read()
    sql2exec=sql.replace('__METRICS_NAME__', args["metrics"]).replace('__START_DATE__', args["start"]).replace('__END_DATE__', args["end"]).replace('__TOP__', args["top"] ).replace('__SCHEMA__', args["schema"]).replace('__STIME__', args["stime"]).replace('__ETIME__', args["etime"])
    ...
    plt.gca().legend(prop=font, loc=9, bbox_to_anchor=(0.5, -0.1),title=" color rank pct sql_id plan_hash_value plan_date cost username ", fancybox=True)
    plt.ylabel(metrics)
    ax = plt.gca()
